...
function removeFirstOccurrences(str, value) {
  ...
  if (indexOfSubstring === -1) return str;

  return str
    .substring(0, indexOfSubstring)
    .concat(str.substring(indexOfSubstring + value.length));
}
...
function removeLastOccurrences(str, value) {
  const indexOfLastOccurrence = str.lastIndexOf(value);
  if (indexOfLastOccurrence === -1) return str;

  return str
    .substring(0, indexOfLastOccurrence)
    .concat(str.substring(indexOfLastOccurrence + value.length));
}
...
function isPalindrome(str) {
  const filtered = str.replaceAll(/\W/gi, '');
  for (let i = 0, j = filtered.length - 1; i < j; i += 1, j -= 1) {
    if (filtered[i].toLowerCase() !== filtered[j].toLowerCase()) return false;
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const indexOfChar = input.indexOf(str[i]);
    if (indexOfChar !== -1) encoded += output[indexOfChar];
    else encoded += str[i];
  }
